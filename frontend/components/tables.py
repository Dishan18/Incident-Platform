...
def render_incident_detail(incident: dict) -> None:
    ...
    if edit_mode:
        ...
        if rca_generated and rca_content:
            ...
            with col_download:
                pdf_blob_name = incident.get("rca_pdf_url")
                if pdf_blob_name:
                    if pdf_blob_name.startswith("http"):
                        pdf_blob_name = pdf_blob_name.split("/")[-1]
                    try:
                        from backend.cloud.azure_blob import download_blob_bytes
                        pdf_bytes = download_blob_bytes(pdf_blob_name, container_name="rca-files")
                        st.download_button(
                            label="Download PDF",
                            data=pdf_bytes,
                        ...
                        st.error(f"Failed to load PDF from secure storage: {e}")
                else:
